...
export function useMapCamera({ mapRef, ready, mode, door, other, others = [], sheetHeight, topInset, reduced, maxSpanM = 40_000 }: Args) {
  ...
  useEffect(() => {
    ...
    if (mode === 'door') { map.animateToRegion({ ...door, latitudeDelta: 0.004, longitudeDelta: 0.004 }, dur); return; }
    if (mode === 'band') { map.animateToRegion({ ...door, latitudeDelta: 0.006, longitudeDelta: 0.006 }, dur); return; }
    if (mode === 'search') {
  ...
  }, [mapRef, ready, mode, door.latitude, door.longitude, otherKey, othersKey, sheetHeight, topInset, reduced, maxSpanM]);
}
