// components/cockpit/Dock.tsx — la barre du bas : liste · état · réglages.
...
export function Dock({ stage, count, onlineSince, missionId, bottom, onList, onSettings }: Props) {
  ...
  return (
    ...
    >
      <Pressable onPress={() => { feedback.haptic('light'); onList(); }} style={[s.ic, { opacity: stage === 'off' || stage === 'busy' ? 1 : 0 }]} accessibilityRole="button" accessibilityLabel={t('cockpit.list')} hitSlop={8} disabled={!(stage === 'off' || stage === 'busy')}>
      ...
      <Pressable onPress={() => { feedback.haptic('light'); onSettings(); }} style={s.ic} accessibilityRole="button" accessibilityLabel={t('cockpit.settings')} hitSlop={8}>
...
const s = StyleSheet.create({
  dock: { position: 'absolute', left: 0, right: 0, zIndex: 6, borderTopWidth: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20 },
  ic: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', zIndex: 2 },
  st: { position: 'absolute', left: 64, right: 64, top: 0, bottom: 0, alignItems: 'center', justifyContent: 'center' },
