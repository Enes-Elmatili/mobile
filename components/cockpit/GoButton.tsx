...
type Props = {
  ...
  /** Taille du disque (72 sur écran bas, 84 sinon) et hauteur du dock. */
  size: number;
  dockHeight: number;
  /** Bord bas du dock (px depuis le bas de l'écran). */
  dockBottom: number;
  /** Zone de contenu : le GO se centre dessus, le stop se pose à son bord gauche. */
  contentLeft: number;
  contentWidth: number;
  onPress: () => void;
...
function GoButtonBase({ shape, size, dockHeight, dockBottom, contentLeft, contentWidth, onPress, accessibilityLabel, accessibilityHint }: Props) {
  ...
  const goLeft = centerX - GO_SIZE / 2;
  const goBottom = dockBottom + dockHeight - GO_SIZE / 2 - 6;
  const stopCenterX = contentLeft + 4 + STOP_SIZE / 2;
  const stopCenterBottom = dockBottom + dockHeight / 2;
  const dx = stopCenterX - centerX;
  ...
  return (
    <Animated.View style={[s.wrap, { width: GO_SIZE, height: GO_SIZE, left: goLeft, bottom: goBottom, shadowOpacity: theme.isDark ? 0.45 : 0.18 }, shell]} pointerEvents={shape === 'hidden' ? 'none' : 'auto'}>
      <Pressable
      ...
      >
        <Animated.View style={[s.inner, press.style]}>
          <Animated.View style={[StyleSheet.absoluteFill, { borderRadius: GO_SIZE / 2 }, disc]} />
          <Animated.View style={[StyleSheet.absoluteFill, s.ring, { borderRadius: GO_SIZE / 2, borderColor: theme.bg }, ring]} pointerEvents="none" />
...
const s = StyleSheet.create({
  wrap: { position: 'absolute', zIndex: 7, shadowColor: '#000', shadowRadius: 18, shadowOffset: { width: 0, height: 10 }, elevation: 10 },
  inner: { flex: 1 },
  ring: { borderWidth: 5 },
