...
// ensuite, sur le même écran — on note ce qu'on vient de voir (spec
// 2026-09-14). La route « rating » est conservée pour les liens profonds et
...
import { providerName } from '@/components/tracking';
...
export default function DoneScreen() {
  ...
  // Le retour physique Android ne doit pas court-circuiter l'écran (le layout
  // parent redirigerait vers l'accueil avant la note).
  useEffect(() => {
  ...
  const submit = async () => {
    ...
    const payload = { providerId: request.providerId, requestId: Number(id), rating, comment: [labels, comment.trim()].filter(Boolean).join(' — ') };
    try { await submitOffline(() => api.post('/ratings', payload), payload as Record<string, unknown>); }
    finally { setSubmitting(false); }
  };

  if (loading || (!request && !loadError)) {
  ...
  return (
    <SafeAreaView style={[s.root, { backgroundColor: theme.bg }]} edges={['top']}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={[s.scroll, { paddingBottom: insets.bottom + 120 }]} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <Reanimated.View style={entrance.style}>
            ...
            <View style={s.facts}>
              ...
              <Pressable style={[s.fact, { backgroundColor: theme.surface }]} onPress={() => router.push({ pathname: '/(tabs)/documents', params: { openRequestId: String(id) } })} accessibilityRole="button" accessibilityLabel={`${t('tracking.fact_receipt')} · ${t('tracking.fact_receipt_where')}`}>
        ...
        <View style={[s.footer, { backgroundColor: theme.bg, borderTopColor: theme.borderLight, paddingBottom: insets.bottom + 12 }]}>
          <Pressable
